...
router.get("/portfolio", (req, res, next) => {
  Coin.find({ addedBy: req.session.currentUser.email }).then(
    (coinsInfoFromDb) => {
      ...
      for (let i = 0; i < p3.length; i++) {
        ...
        for (let j = 0; j < coinsInfoFromDb.length; j++) {
          ...
          if (firstListObject.name == portfolioListObject.name) {
            totalPortfolioValue +=
              firstListObject.current_price * portfolioListObject.owned;
            totalPortfolioValue = Math.round(totalPortfolioValue * 100) / 100;
            totalSpent +=
              Math.round(
                portfolioListObject.purchasedAt *
                  portfolioListObject.owned *
                  100
              ) / 100;

            let newObj = {
              ...portfolioListObject.toObject(),
              price: firstListObject.current_price,
              currentValue:
                firstListObject.current_price * portfolioListObject.owned,
            };
      ...
      differencePercent = (
        ((totalPortfolioValue - totalSpent) / totalSpent) *
        100
      ).toFixed(0);
...
router.get("/news", (req, res) => {
  axios
    .get(
      "https://newsapi.org/v2/everything?q=crypto&sortBy=publishedAt&language=en&apiKey=42a0cfb20870473e851473bcecc219e0"
    )
    .then((response) => {
...
router.get("/watchlist", (req, res, next) => {
  Watchlist.find({ watchAddedBy: req.session.currentUser.email }).then(
    (watchlistCoinsFromDb) => {
      let resultsArr = [];
      ...
      for (let i = 0; i < p3.length; i++) {
        ...
        for (let j = 0; j < watchlistCoinsFromDb.length; j++) {
          ...
          if (firstListObject.name == portfolioListObject.watchCoinName) {
            let newObj1 = {
              ...portfolioListObject.toObject(),
              currPrice: firstListObject.current_price,
            };
